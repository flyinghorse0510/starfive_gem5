...
NUM_LLC=16
NUM_MEM=1
DEBUG_FLAGS=ProdConsMemLatTest,RubyGenerated,TxnTrace #RubyCHIDebugStr5,RubyGenerated
DCT_CONFIGS=(False) #(False True) #(False True) #(True False)
NETWORK="simple"
...
if [ "$C2CBW" != "" ]; then
  ...
  PRODUCER_SET_CONFIGS=(1) #(1 2 4 8) #$(seq 0 $((${NUM_CPUS}-1))) #("0") #(1 2 4 8)
  WKSETLIST=(1024) #(65536) #(65536 131072) #(1024 65536)
  OUTPUT_PREFIX="PRODCONS_BW"
  
  for DCT in ${DCT_CONFIGS[@]}; do
    for NUM_PRODUCERS in ${PRODUCER_SET_CONFIGS[@]}; do
      for NUM_CONSUMERS in ${CONSUMER_SET_CONFIGS[@]}; do
        if [[ $(($NUM_PRODUCERS + $NUM_CONSUMERS)) -lt ${NUM_CPUS} ]]; then
          for WKSET in ${WKSETLIST[@]}; do
            OUTPUT_DIR="${OUTPUT_ROOT}/${OUTPUT_PREFIX}/WS${WKSET}_Core${NUMCPUS}_Prod${PRODUCER_SET}_Cons${CONSUMER_SET}_L1${l1d_size}_L2${l2_size}_L3${l3_size}_DCT${DCT}"
            echo $OUTPUT_DIR
            mkdir -p $OUTPUT_DIR
      ...
      for CONSUMER_SET in ${CONSUMER_SET_CONFIGS[@]}; do
        if [[ $(($PRODUCER_SET + $CONSUMER_SET)) -lt ${NUM_CPUS} ]]; then
          for WKSET in ${WKSETLIST[@]}; do
            OUTPUT_DIR="${OUTPUT_ROOT}/${OUTPUT_PREFIX}/WS${WKSET}_Core${NUMCPUS}_Prod${PRODUCER_SET}_Cons${CONSUMER_SET}_L1${l1d_size}_L2${l2_size}_L3${l3_size}_DCT${DCT}"
            echo $OUTPUT_DIR
            # grep -E 'system\.cpu01\.data_sequencer|l2\.snpIn|cntrl.snpOut' $OUTPUT_DIR/debug.trace > $OUTPUT_DIR/debug.ReqAnalyses.trace
            # grep -E 'txsn: 0x001800000000001b' $OUTPUT_DIR/debug.trace > $OUTPUT_DIR/debug.ReqAnalyses.trace
            # grep -E 'Req Begin|Req Done|Start,R' $OUTPUT_DIR/debug.trace > $OUTPUT_DIR/debug.ReqAnalyses.trace
            # grep -E 'Start,R' $OUTPUT_DIR/debug.trace > $OUTPUT_DIR/debug.ReqAnalyses.trace
            grep -E 'Start,R|Start,W' $OUTPUT_DIR/debug.trace > $OUTPUT_DIR/debug.ReqAnalyses.trace
            # grep -E 'txsn: 0x0003000000000012|event: AllocRequest, addr: 0x80' $OUTPUT_DIR/debug.trace > $OUTPUT_DIR/debug.ReqAnalyses.trace
            # grep -E 'txsn: 0x0003000000000012|event: AllocRequest, addr: 0x80' $OUTPUT_DIR/debug.trace > $OUTPUT_DIR/debug.ReqAnalyses.trace
            # grep -E 'system.ruby.hnf02.cntrl:  addr: 0x80'
            # grep 'Req Done' $OUTPUT_DIR/debug.trace > $OUTPUT_DIR/debug.ReqAnalyses.trace
          done
        fi
      done
    done
