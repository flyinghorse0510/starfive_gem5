...
void bufferCopy(unsigned int num0, unsigned int num1)
{
	...
	unsigned int mask = (1 << ((BUFFER_NUM_BITS + 1) >> 1)) - 1;
	int i;

	for(i = 0; i < 4096; i++)
	{
		unsigned int index = step & mask;
		unsigned short value = (unsigned short)buffer0[addr] + (unsigned short)buffer0[4095 - addr];
		glbBuffers[index][addr] = glbBuffers[BUFFER_NUMBER - 1 - index][value];
		addr = (addr + step) & 0xfff;
		step = (step + num0) % num1;
	}
}
