...
WKSET=262144 #262144 #131072 #8192 #16384 #524288 #(32768) #
#NUM_MEM_SET=(1 2)
NUM_MEM=1
TRANS_SET=(1 2 4)
SNF_TBE_SET=(32)
...
#NUM_LOAD_SET=(100)
NUM_LOAD_SET=(50) #(50)
MESHES=(CustomMesh CustomMesh_4Vnet)
...
ROUTER_LAT=0
VC_PER_VNET=2
build/RISCV_CHI/mem/ruby/system/Sequencer.cc:619: warn: Replacement policy updates recently became the responsibility of SL
PHYVNET=True #True #False
#

INJ_INTV_SET=(3 4 8 16 20 24 32)
INJ_INTV_SET=(32 24 20 16 8 4 3)
#INJ_INTV_SET=(2)

# DEBUG_FLAGS="SeqMemLatTest,TxnTrace"
#EBUG_FLAGS="SeqMemLatTest,TxnTrace,TxnLink"
    ...
    for DMT in ${DMT_Config[@]}; do
       for NUMCPUS in ${NUM_CPU_SET[@]}; do
          for TRANS in ${TRANS_SET[@]}; do
            ...
            OUTPUT_DIR="${OUTPUT_ROOT}/${OUTPUT_PREFIX}/WS${WKSET}_Core${NUMCPUS}_L1${l1d_size}_L2${l2_size}_L3${l3_size}_MEM${NUM_MEM}_MEMLOC${NUM_DDR_XP}Side${NUM_DDR_Side}_INTERLV${MultiCoreAddrMode}_SNFTBE${SNF_TBE}_DMT${DMT}_TRANS${TRANS}_NUMLOAD${NUM_LOAD}_INJINTV${INJ_INTV}" 
  ...
  if [ "$TXNTRACE" != "" ]; then
    ...
    for DMT in ${DMT_Config[@]}; do
       for NUMCPUS in ${NUM_CPU_SET[@]}; do
          ...
          grep -E 'ReqBegin|ReqDone|requestToMemory|responseFromMemory' ${OUTPUT_DIR}/debug.trace > ${OUTPUT_DIR}/simple.trace 
          python3 simple_parser.py --input ${OUTPUT_DIR}
             done
            done
           done
         done
...
#                   for NUM_DDR_XP in ${NUM_DDR_XP_SET[@]}; do 
          ...
          python3 stats_parser.py --input ${OUTPUT_DIR}/stats.txt --output ${OUTPUT_DIR}/stats.log --num_cpu ${NUMCPUS} --num_llc ${NUM_LLC} --num_ddr ${NUM_MEM} --trans ${TRANS} --snf_tbe ${SNF_TBE} --dmt ${DMT} --linkwidth ${LINKWIDTH} --injintv ${INJ_INTV} --print l1d,l1i,l2p,llc,cpu,ddr  #True #False
                   done
#                   mv throughput.txt throughput_MEMLOC${NUM_DDR_Side}.txt  
    ...
    mv throughput.txt.csv throughput_MEMLOC${NUM_DDR_XP}_PHYVNET${PHYVNET}.csv
done
fi
