...
if [ "$GATETEST" != "" ]; then
    ...
    XOR_ADDR_BITS_SET=(False)
    BLOCK_STRIDE_CONFIG_SET=(0 2)
                            ...
                            for SNOOP_FILTER_ASSOC in ${SNOOP_FILTER_ASSOC_CONFIG_SET[@]}; do
                                for BLOCK_STRIDE_BITS in ${BLOCK_STRIDE_CONFIG_SET[@]}; do
                                    for XOR_ADDR_BITS in ${XOR_ADDR_BITS_SET[@]}; do
                                        OUTPUT_DIR="${OUTPUT_ROOT}/${OUTPUT_PREFIX}/WS${WKSET}_Core${NUMCPUS}_NumLLC${NUM_LLC}_L1${l1d_size}_L2${l2_size}_L3${l3_size}_DMT${DMT}_DCT${DCT}_L2Assoc${l2_assoc}_XorAddrBits${XOR_ADDR_BITS}_BlockStrideBits${BLOCK_STRIDE_BITS}"
